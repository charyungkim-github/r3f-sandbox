...
import { uniforms, fragmentShader } from './shaders/clippingShader.js'

export default function Map(props) {

  const { nodes, materials } = useGLTF('/models/map.glb')
  useEffect(()=>{ attachClipping(materials) }, [])
...
function attachClipping(materials, config) {
  ...
  const clippingConfig = config ? config : defaultConfig

  Object.keys(materials).map((key) => {

    // skip if disabled
    // to disable clipping, use 'material-clipping={false}'
    if(materials[key].clipping == false) return

    materials[key].onBeforeCompile = (shader) => {

      // init uniforms
      shader.uniforms.near = { value: clippingConfig.near }
      shader.uniforms.far = { value: clippingConfig.far }
      shader.uniforms.size = { value: clippingConfig.size }

      // add shader
      shader.fragmentShader = uniforms + shader.fragmentShader.replace('#include <color_fragment>', fragmentShader )
    }
  })
}